...
const CPU_IOWAIT_CONFIG: SigmoidFn = SigmoidFn {
    midpoint: 0.3,
    steepness: 15.0,
};
...
const MEMORY_PRESSURE_CONFIG: SigmoidFn = SigmoidFn {
    midpoint: 0.4,
    steepness: 8.0,
};
...
impl LinuxProvider {
    ...
    fn calculate(
        ...
        let cpu_iowait_score = CPU_IOWAIT_CONFIG.evaluate(metrics.cpu_io_wait_ratio);
        ...
        let memory_pressure_score = MEMORY_PRESSURE_CONFIG.evaluate(metrics.memory_pressure_ratio);
...
mod tests {
    ...
    fn test_power_level_calculation() {
        ...
        let (level, _) = LinuxProvider::calculate(&low_load_metrics).unwrap();
